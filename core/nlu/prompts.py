class Prompts:
    ...
    extract_intent_entities_prompt = (
        "당신은 한국어 NLU 분석기입니다. 도움이 되고 자연스러운 한국어 챗봇 어시스턴트 롤을 수행하기 위해 당신은 사용자가 명시한 GraphInfo 상의 노드 흐름을 따라 최종적으로는 마지막 노드까지 대화 상대를 진행 시키는것이 목적입니다.  사용자 발화에서 의도(intent), 엔티티(entities), \n"
    ...
    # NLG Prompt
    generate_response_system_prompt = """당신은 도움이 되고 자연스러운 한국어 챗봇 어시스턴트입니다. 당신은 사용자가 생성한 Graph의 노드 흐름을 따라 마지막 노드까지 유저를 진행시키는것이 목적입니다. 
    ...
    def build_nlg_user_prompt(context: dict, node_config: dict, intent_data: dict = None) -> str:
        ...
        if intent_data:
            if intent_data.get('intent') == 'off_topic':
                ...
                scenario_info = "\n모든 필요한 정보가 수집되었습니다. 확인 및 다음 단계 안내를 해주세요. 스테이지도 slot_filling stage의 완료를 시스템에게 넘겨주세요"
            elif current_stage == 'fallback':
                scenario_info = "\n사용자가 지정한 Fallback Scenario에 따른 응답을 생성해주세요."
            elif intent_data.get('intent') == 'goodbye':
                scenario_info = "\n사용자가 종료 의도를 표시했습니다. 친절하게 종료 메시지를 제시해주세요."
